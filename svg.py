...
def load_images(word):
    return soup.search('logo ' + word + ' -icon', image_type='svg', n_images=12345)
...
def save(image_data):
    if image_data['ou'] is not None:
        ...
        path = os.path.join(IMAGES_DEST, 'svg', filename)
        ...
        if not os.path.exists(path):
            ...
            with open(os.path.join(IMAGES_DEST, 'meta', filename+'.txt'), 'w') as f:
                f.write(str(image_data))
